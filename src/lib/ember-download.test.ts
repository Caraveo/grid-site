...
import {
  chooseEmberDownload,
  type EmberBrowserSignals,
} from "./ember-download";

const mac: EmberBrowserSignals = {
  userAgent:
...
test("selects the Intel macOS release from client hints", () => {
  const choice = chooseEmberDownload({ ...mac, architecture: "x86" });
...
test("selects the Apple silicon release from client hints", () => {
  const choice = chooseEmberDownload({ ...mac, architecture: "arm" });
...
test("uses Apple GPU evidence when client hints are unavailable", () => {
  const choice = chooseEmberDownload({
    ...mac,
...
test("does not guess when a Mac browser hides the architecture", () => {
  const choice = chooseEmberDownload(mac);
  assert.equal(choice.label, "Get Wallet");
  assert.equal(choice.href, "https://grid-compute.com/wallet");
