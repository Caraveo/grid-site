...
import {
  detectPhoenixDownload,
  phoenixFallback,
  type PhoenixDownloadChoice,
} from "@/lib/phoenix-download";

export function PhoenixDownload() {
  const [download, setDownload] =
    useState<PhoenixDownloadChoice>(phoenixFallback);

  useEffect(() => {
    let active = true;
    void detectPhoenixDownload().then((choice) => {
  ...
  return (
    <div className="mt-10 flex flex-col items-center">
      ...
      {download.mobile ? (
        <p className="mt-3 max-w-md text-sm leading-relaxed text-muted">
          Phoenix mobile is coming soon. Choose a desktop build on the Phoenix page.
        </p>
      ) : null}
